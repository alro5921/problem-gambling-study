...
def plot_compare_bar(ax, data, title, ylab):
    ...
    ax.bar(names, data, width=.5, color=['#B22222', '#228B22'])
    ax.set_title(title, size=40, color='k')
    ax.set_ylabel(ylab, size=24, color='k')
    ax.tick_params(axis="y", labelsize=20, color='k')
    ax.tick_params(axis="x", labelsize=30, color='k')
